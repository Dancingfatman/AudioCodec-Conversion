...
def getOutputFileName(inputFileName):
	...
	return f'{filename}_convert{file_extension}'
...
#Check if audio stream needs converting
def needsConvert(ffmpegDir, dirPath, inputFileName):
	if checkExtension(inputFileName) == False:
		return False

	outputName = getOutputFileName(inputFileName)
	if os.path.exists(outputName):
		return False

	cmd = f'"{ffmpegDir}ffprobe -v error -select_streams a:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 "{inputFileName}"'
...
def convert(ffmpegDir, inputFileName):
	...
	convertCmd = f'"{ffmpegDir}ffmpeg" -i "{inputFileName}" -loglevel warning -hide_banner -nostats -vcodec copy -acodec aac -ac 2 -af \"pan=stereo|FL=FC+0.30*FL+0.30*BL|FR=FC+0.30*FR+0.30*BR\" "{outputName}"'
